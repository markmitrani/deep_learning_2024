...
class Elman(nn.Module):
    def __init__(self, insize=300, outsize=300, hsize=300):
        ...
        # Fully connected layers for input and hidden state
        self.lin1 = nn.Linear(insize + hsize, hsize)  # Input + hidden state -> new hidden state
        self.lin2 = nn.Linear(hsize, outsize)        # Hidden state -> output
        
        self.activation = nn.Tanh()  # Activation function for hidden state

    def forward(self, x, hidden=None):
        b, t, e = x.size()  # Batch size, time steps, embedding size
        device = x.device   # Ensure all tensors are on the same device
        
        if hidden is None:
            hidden = torch.zeros(b, self.lin1.out_features, dtype=torch.float, device=device)
        ...
        for i in range(t):
            ...
            hidden = self.activation(self.lin1(inp))
            
            # Compute output
            out = self.lin2(hidden)
            outs.append(out[:, None, :])  # Add time dimension back
        
        # Concatenate outputs across all time steps
